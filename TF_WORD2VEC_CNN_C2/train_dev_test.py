...
tf.flags.DEFINE_integer("sequence_length", 100, "the max of words in a sentence")
...
if FLAGS.cxt_type == "NC":
    ...
    logging.basicConfig(level=logging.CRITICAL, filename='C2-NC-CNN.log', filemode='w')
elif FLAGS.cxt_type == "LC":
    network_input_width = FLAGS.embedding_dim * 2
    logging.basicConfig(level=logging.CRITICAL, filename='C2-LC-CNN.log', filemode='w')
elif FLAGS.cxt_type == "RC":
    network_input_width = FLAGS.embedding_dim * 2
    logging.basicConfig(level=logging.CRITICAL, filename='C2-RC-CNN.log', filemode='w')
elif FLAGS.cxt_type == "LRC":
    network_input_width = FLAGS.embedding_dim * 3
    logging.basicConfig(level=logging.CRITICAL, filename='C2-LRC-CNN.log', filemode='w')
...
x_data, y_data = data_helper.load_data_label(FLAGS.positive_data_file, FLAGS.negtive_data_file, FLAGS.w2vModelPath,
...
SKF = KFold(len(y_data), n_folds=10, shuffle=True, random_state=42)
for train_indices, test_indices in SKF:
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            num_neg, num_pos = 0, 0
            num_neg_pos, num_neg_neg = 0, 0
            num_pos_pos, num_pos_neg = 0, 0
            ...
            if (num_pos_neg + num_neg_neg) != 0:
                P_neg = num_neg_neg / (num_pos_neg + num_neg_neg)
            else:
                P_neg = 0.0
            if num_neg != 0:
            ...
            if (num_pos_pos + num_neg_pos) != 0:
                P_pos = num_pos_pos / (num_pos_pos + num_neg_pos)
            else:
                P_pos = 0.0
            if num_pos != 0:
            ...
            print("CV {}: P_neg: {:.4f}, R_neg: {:.4f}, F_neg: {:.4f}, P_pos: {:.4f}, R_pos: {:.4f}, F_pos: {:.4f}, Test Accuracy: {:.4f}\n".format(CV, P_neg, R_neg, F_neg, P_pos, R_pos, F_pos, test_accuracy))
            logging.critical("CV {}: P_neg: {:.4f}, R_neg: {:.4f}, F_neg: {:.4f}, P_pos: {:.4f}, R_pos: {:.4f}, F_pos: {:.4f}, Test Accuracy: {:.4f}\n".format(CV, P_neg, R_neg, F_neg, P_pos, R_pos, F_pos, test_accuracy))
            CV += 1
